...
We do so using the algorithm described in algorithm~\ref{random_generation} in Appendix~\ref{codeappendix}.
In this algorithm, \texttt{rmatrix} denotes the matrix of all correlations between the different genes in the motif.
The content of this variable is produced by the \texttt{GenerateCorrelations} function (algorithm~\ref{generate_correlations}).
...
The algorithm is tweaked slightly in order to not exclude some realistic biological networks.
First, we generate a list of all possible edges using the \texttt{FindEdges} function.
We allow cyclical networks, which we realize by allowing a new edge to be either incoming or outgoing, instead of always making it an outgoing edge.
We allow self-loops, as well as many-to-one edges.
...
As all edges have one target, we define the desired number of edges as the average indegree of the system.
When adding an edge, we shuffle the list of possible edges.
We then go through this list until we encounter an edges that contains the node that is being added, and that contains no nodes that have not been added yet.
A downside of this method is that this heavily favors many-to-one edges, as these are more numerous.
To prevent this, this function can be tweaked to prefer one-to-one edges over many-to-one edges, as these edges are much more common in real life than the many-to-one variant.
...
We pass our motifs, a list of variables that are to be nudged, and a nudge size $0 \le \epsilon \le 1$ that represents the fraction of the total probability that should be moved.
...
For an upper bound, we use the maximum MI between a single element of the system and the entire system
%
\begin{equation}
\max_i (H(x_i, X))
\end{equation}
...
This boundary cannot be exceeded, as % TODO: ask Rick, ook in lit, potentially switch to I-max synergy
