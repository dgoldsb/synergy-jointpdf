function [results]=testYeastModel(model,varargin)
    ...
    if output
        ...
        if strcmpi(model.description, 'bmid000000141353')
            ...
            fprintf('\n%s\n\t%s\t%s\n\n\t%g\t%s\n\t%g\t%s\n\t%4g\t%s\n\t%4g\t%s\n\n',...
                ...
                sum(cellfun('isempty',model.rules)), ...
    ...
    %% dubious ORFs
        if strcmpi(model.description, 'bmid000000141353')
            ...
            if output
                fprintf('\t%4g\t(%.2f%%)\t%s\n',length(dubiousORFs),...
                    (100*length(dubiousORFs)/gene_number),'dubious ORFs');
            end
    ...
    if medium == 1
        ...
        switch lower(model.description)
            ...
            case {'bmid000000141353'}
                % includes L-tyrosine (met bigg_tyr_L_bm), L-lysine (met
                % bigg_lys_L_bm), L-isoleucine (met bigg_ile_L_bm),
                % L-arginine (met bigg_arg_L_bm), L-histidine (met
                % bigg_his_L_bm), L-methionine (met bigg_met_L_bm), and
                % L-tryptophan (bigg_trp_L_bm) to enable growth.
	...
	if medium == 2
        ...
        switch lower(model.description)
            ...
            case {'yeast_7.00_cobra'}
                model = complete_Y7(model);
                
            case {'bmid000000141353'}
                model = complete_bio(model);
     ...
     switch lower(model.description)
        ...
        case {'ito977_cobra_compatible'}
            ...
            uptake_rxn_ids = {'2MBACxtI';'2MBALDxtI';'44DIMZYMSTxtI';...
                ...
                'ATNxtI';'ATTxtI';...
                %'BMxtI';... Don't allow biomass uptake
                'BTxtI';'C100xtI';'C120xtI';...
    ...
    %% find blocked reactions (cannot carry any flux per FVA) -- function moved
    ...
    %% find dead end metabolites -- function moved
...
%    mod_inviable = model.genes(grRatio < ko_tol | isnan(grRatio)); % use if FBA failure = inviable
...
% 
%         if strcmpi(model.description, 'iTO977_cobra_compatible')
%             % for iTO, need to skip YCR098C when all exchange reactions allowed
%             % b/c that simulation hangs matlab.
%             geneList = model.genes(~strcmp(model_maximal.genes,'YCR098C'));
%             grRatio = singleGeneDeletion(model_maximal,'FBA',geneList);
%         else
...
function model = complete_bio(model)
    % change biomodels.db model medium to the Kennedy synthetic complete
    % medium NOTE: biomodels doesn't have exchange reactions for
    % octadecenoate or octadecynoate.
    
    % start with a clean slate: set all exchange reactions to unconstrained
    % excretion, no uptake.
    ...
    constrainedUptake_out = {'bigg_4abz_out'; 'bigg_ade_out'; ...
        'bigg_ala_L_out'; 'bigg_arg_L_out'; 'bigg_asn_L_out'; ...
        'bigg_aspsa_out'; 'bigg_btn_out'; 'bigg_cys_L_out'; ...
        'bigg_dhf_out'; 'bigg_ergst_out'; 'bigg_gln_L_out'; ...
        'bigg_glu_L_out'; 'bigg_gly_out'; 'bigg_hdcea_out'; ...
        'bigg_his_L_out'; 'bigg_ile_L_out'; 'bigg_inost_out'; ...
        'bigg_ile_L_out'; 'bigg_lys_L_out'; 'bigg_met_L_out'; ...
        'bigg_nac_out'; 'bigg_phe_L_out'; 'bigg_pnto_R_out'; ...
        'bigg_pro_L_out'; 'bigg_pydxn_out'; 'bigg_ribflv_out'; ...
        'bigg_ser_L_out'; 'MNXM97132_out'; 'bigg_thr_L_out'; ...
        'bigg_trp_L_out'; 'bigg_tyr_L_out'; 'bigg_ura_out'; ...
        'bigg_val_L_out'; 'bigg_zymst_out';};
    
    glucoseExchange = {'MNXM99_in'};
    
    unconstrainedUptake_in = {'MNXM2_in'; 'MNXM15_in'; 'MNXM9_in'; ...
        'MNXM58_in'; 'MNXM27_in'; 'MNXM95_in'; 'MNXM13_in'; 'MNXM4_in'};
 
    constrainedUptakeRxnIndexes_out = ...
        findRxnIDs(model,constrainedUptake_out);

    glucoseExchangeIndex = findRxnIDs(model,glucoseExchange);

    unconstrainedUptakeRxnIndexes_in = ...
        findRxnIDs(model,unconstrainedUptake_in);
    
    model.lb(constrainedUptakeRxnIndexes_out) = -0.5;
    model.ub(glucoseExchangeIndex) = 20;
    model.ub(unconstrainedUptakeRxnIndexes_in) = 1000;
...
function genes = auxotrophicORFs
    ...
    %    'YER023W';...%added by bh 10/15/12 - per SGD, pro1, pro2, pro3 are auxotrophs however: "A unique property of all the pro mutant strains is that they cannot grow on standard YPD rich media."
        ...
        'YBR252W';... %added by bh 12/11/12 - per SGD, mutation leads to dTMP auxotrophy - on essential list
        'YDL205C';... %added by bh 12/11/12 - per SGD, mutation leads to heme auxotrophy - on essential list
        'YDR047W';... %added by bh 12/11/12 - per SGD, mutation leads to heme auxotrophy - on essential list
        'YDR232W';... %added by bh 12/11/12 - per SGD, mutation leads to heme auxotrophy - on essential list
        %'YFR019W';... %removed by BH 12/11/12 - temp sensitive inositol auxotroph
        'YGL040C';... %added by bh 12/11/12 - per SGD, mutation leads to heme auxotrophy - on essential list
        %'YGL062W';... %removed by BH 12/11/12 - temp sensitive inositol auxotroph
        'YGR065C';... %added by bh 12/11/12 - per SGD, mutation leads to fatty acid auxotrophy - on essential list
        'YGR267C';... %added by bh 12/11/12 - per SGD, mutation leads to folinic acid auxotrophy - on essential list
        'YHR128W';... %added by bh 12/11/12 - per SGD, mutation leads to uracil and uridine auxotrophy - on essential list
        'YMR113W';... %added by bh 12/11/12 - per SGD, mutation leads to folinic acid auxotrophy - on essential list
        'YNL256W';... %added by bh 12/11/12 - per SGD, mutation leads to folic acid auxotrophy - on essential list
        'YOR278W';... %added by bh 12/11/12 - per SGD, mutation leads to heme auxotrophy - on essential list
        'YGL055W';... %added by bh 12/13/12 - per SGD, mutation leads to oleic acid auxotrophy (probably monounsaturated fatty acids in general) - on essential list
