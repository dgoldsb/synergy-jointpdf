% Master script to run model tests and save the results for further analysis

%% first, load the models for comparison
load('../models/models_as_used/iFF_bh.mat'); % doi:10.1101/gr.234503
load('../models/models_as_used/iND_bh.mat'); % doi:10.1101/gr.2250904
load('../models/models_as_used/iIN800_bh.mat'); % from Markus Herrgard on 6/24/2013, doi:10.1186/1752-0509-2-71
load('../models/models_as_used/iMM_bh.mat'); % doi:10.1186/1752-0509-3-37
load('../models/models_as_used/Y4_bh.mat'); % 4.05 from yeast.sf.net doi:10.1186/1752-0509-4-145
load('../models/models_as_used/iAZ_bh.mat'); % doi:10.1186/1752-0509-4-178
load('../models/models_as_used/bs_bh.mat'); % doi:10.1038/ng.846
load('../models/models_as_used/Y5_bh.mat'); % 5.01 from yeast.sf.net doi:10.1186/1752-0509-6-55
load('../models/models_as_used/iTO_bh.mat'); % from Markus Herrgard on 6/24/2013, modified by bh to add gene info f: 0.8333 doi:10.1186/1752-0509-7-36
load('../models/models_as_used/Y6_bh.mat'); % 6.06 from yeast.sf.net doi:10.1093/database/bat059
load('../models/models_as_used/Y7_bh.mat'); % 7.0 from yeast.sf.net doi:10.1089/ind.2013.0013.
load('../models/models_as_used/bio_bh.mat'); % downloaded 11/21/13 from https://www.ebi.ac.uk/biomodels-main/BMID000000141353

%% Conduct growth rate simulations
% apply constraint to simulate growth with a single limiting nutrient
% (Carbon or Nitrogen), with iFF biomass and minimal medium

testGrowthRate(iFF,1) 
testGrowthRate(iND,1) 
testGrowthRate(iIN,1)
testGrowthRate(iMM,1)
testGrowthRate(Y4,1) 
testGrowthRate(iAZ,1)
testGrowthRate(bs,1) 
testGrowthRate(Y5,1) 
testGrowthRate(iTO,1)
testGrowthRate(Y6,1) 
testGrowthRate(Y7,1) 
testGrowthRate(bio,1) 

% apply multiple constraints for single limiting nutrient (Carbon and
% oxygen or Nitrogen, Carbon, and oxygen), with iFF biomass and minimal
% medium

testGrowthRate(iFF,2) 
testGrowthRate(iND,2) 
testGrowthRate(iIN,2) 
testGrowthRate(iMM,2) 
testGrowthRate(Y4,2) 
testGrowthRate(iAZ,2)
testGrowthRate(bs,2) 
testGrowthRate(Y5,2) 
testGrowthRate(iTO,2)
testGrowthRate(Y6,2)
testGrowthRate(Y7,2) 
testGrowthRate(bio,2) 

% the following are if I set a single limiting nutrient, with the model-supplied objective function:
testGrowthRateDefaultObjective(iFF,1) 
testGrowthRateDefaultObjective(iND,1)
testGrowthRateDefaultObjective(iIN,1)
testGrowthRateDefaultObjective(iMM,1)
testGrowthRateDefaultObjective(Y4,1) 
testGrowthRateDefaultObjective(iAZ,1) 
testGrowthRateDefaultObjective(bs,1) 
testGrowthRateDefaultObjective(Y5,1)
testGrowthRateDefaultObjective(iTO,1) 
testGrowthRateDefaultObjective(Y6,1) 
testGrowthRateDefaultObjective(Y7,1) 
testGrowthRateDefaultObjective(bio,1) 

% the following are if I set multiple constraints, with the model-supplied objective function:
testGrowthRateDefaultObjective(iFF,2) 
testGrowthRateDefaultObjective(iND,2) 
testGrowthRateDefaultObjective(iIN,2) 
testGrowthRateDefaultObjective(iMM,2) 
testGrowthRateDefaultObjective(Y4,2) 
testGrowthRateDefaultObjective(iAZ,2) 
testGrowthRateDefaultObjective(bs,2)
testGrowthRateDefaultObjective(Y5,2) 
testGrowthRateDefaultObjective(iTO,2)
testGrowthRateDefaultObjective(Y6,2)
testGrowthRateDefaultObjective(Y7,2) 
testGrowthRateDefaultObjective(bio,2) 

%% modify models to remove reversible aspartate mitochondrial transport reaction and test
iNDm = iND;
iINm = iIN;
iMMm = iMM;
Y4m = Y4;
...
iNDm.lb(187)=0;iNDm.ub(187)=0;
iINm.lb(39) = 0;iINm.ub(39) = 0;
iMMm.lb(414)=0;iMMm.ub(414)=0;
Y4m.lb(1163) = 0;Y4m.ub(1163);
iAZm.lb(251)=0;iAZm.ub(251)=0;
bsm.lb(248)=0;bsm.ub(248)=0;
Y5m.lb(1117) = 0;Y5m.ub(1117) = 0;
iTOm.lb(51) = 0;iTOm.ub(51) = 0;
Y6m.lb(1048) = 0;Y6m.ub(1048) = 0;
Y7m.lb(877) = 0;Y7m.ub(877) = 0;


testGrowthRate(iNDm,2) 
testGrowthRate(iINm,2) 
testGrowthRate(iMMm,2) 
testGrowthRate(Y4m,2) 
testGrowthRate(iAZm,2)
testGrowthRate(bsm,2) 
testGrowthRate(Y5m,2) 
testGrowthRate(iTOm,2)
testGrowthRate(Y6m,2)
testGrowthRate(Y7m,2) 

%% look for genes unique to each model
% don't include bs since it is the same as iMM

% preprocess biomodels since it has different format
bio.genes  = regexprep(bio.genes,'_i','');
    bio.genes  = regexprep(bio.genes,'_\d','');
    metaCyc_genes = ~cellfun('isempty',regexp(bio.genes,'MetaCyc'));
    bio.genes(metaCyc_genes) = {' '};
    
iFF_unique = setdiff(iFF.genes, ...
    unique([iND.genes; iIN.genes; iMM.genes; iAZ.genes; Y4.genes; ...
    Y5.genes; Y6.genes; iTO.genes; Y7.genes; bio.genes])); % 4

iND_unique = setdiff(iND.genes, ...
    unique([iFF.genes; iIN.genes; iMM.genes; iAZ.genes; Y4.genes; ...
    Y5.genes; Y6.genes; iTO.genes; Y7.genes; bio.genes])); % 0
...
Y4_unique = setdiff(Y4.genes, ...
    ...
    Y5.genes; Y6.genes; iTO.genes; Y7.genes; bio.genes])); % 19

iAZ_unique = setdiff(iAZ.genes, ...
    unique([iND.genes; iIN.genes; iMM.genes; iFF.genes; Y4.genes; ...
    Y5.genes; Y6.genes; iTO.genes; Y7.genes; bio.genes])); % 2

Y5_unique = setdiff(Y5.genes, ...
    unique([iND.genes; iIN.genes; iMM.genes; iAZ.genes; Y4.genes; ...
    iFF.genes; Y6.genes; iTO.genes; Y7.genes; bio.genes])); % 0

iTO_unique = setdiff(iTO.genes, ...
    unique([iND.genes; iIN.genes; iMM.genes; iAZ.genes; Y4.genes; ...
    Y5.genes; Y6.genes; iFF.genes; Y7.genes; bio.genes])); % 35

Y6_unique = setdiff(Y6.genes, ...
    unique([iND.genes; iIN.genes; iMM.genes; iAZ.genes; Y4.genes; ...
    Y5.genes; iFF.genes; iTO.genes; Y7.genes; bio.genes])); % 0

Y7_unique = setdiff(Y6.genes, ...
    unique([iND.genes; iIN.genes; iMM.genes; iAZ.genes; Y4.genes; ...
    Y5.genes; iFF.genes; iTO.genes; Y6.genes; bio.genes])); % 0

bio_unique = setdiff(Y6.genes, ...
    unique([iND.genes; iIN.genes; iMM.genes; iAZ.genes; Y4.genes; ...
    Y5.genes; iFF.genes; iTO.genes; Y7.genes; Y6.genes])); % 0
...
Y7_dubious_unique = setdiff(Y7_unique, verified); % 0
bio_dubious_unique = setdiff(bio_unique, verified); % 0

%% save results of unique gene analysis
save('results/unique_genes','-regexp','.+_unique')
clear -regexp .+_unique

%% blocked reactions -- requires fastFVA, so commented out by default
...
% Y7_blocked = find_dead_ends_and_blocked(Y7);
% bio_blocked = find_dead_ends_and_blocked(bio);
...
iTO_default_media = testYeastModel(iTO,0,0,0,0,1); %hangs - am I using right ITO?
...
Y7_default_media = testYeastModel(Y7,0,0,0,0,1);
Y7_default_media_iFF_biomass = testYeastModel(Y7,0,1,0,0,1);
bio_default_media = testYeastModel(bio,0,0,0,0,1);
% bio model doesn't predict biomass production with iFF def, so don't do.
%bio_default_media_iFF_biomass = testYeastModel(bio,0,1,0,0,1); 

save('results/single_analysis','-regexp','.+default_media', '-append')
...
save('results/single_analysis','-regexp','.+minimal_media', '-append')
Y7_minimal_media = testYeastModel(Y7,1,0,0,0,1);
Y7_minimal_media_iFF_biomass = testYeastModel(Y7,1,1,0,0,1);
save('results/single_analysis','-regexp','.+minimal_media', '-append')
bio_minimal_media = testYeastModel(bio,1,0,0,0,1);
% bio model doesn't predict biomass flux with iFF def, so don't do it.
% bio_minimal_media_iFF_biomass = testYeastModel(bio,1,1,0,0,1); 
save('results/single_analysis','-regexp','.+minimal_media', '-append')
...
iTO_sc_media_iFF_biomass = testYeastModel(iTO,2,1,0,0,1); % note: due to bug in Gurobi 5.60 Simplex post-solver, need to relax tolerances in getCobraSolverParams, or this hangs
Y6_sc_media = testYeastModel(Y6,2,0,0,0,1);
Y6_sc_media_iFF_biomass = testYeastModel(Y6,2,1,0,0,1);
Y7_sc_media = testYeastModel(Y7,2,0,0,0,1);
Y7_sc_media_iFF_biomass = testYeastModel(Y7,2,1,0,0,1); % note: many NAN solutions returned by glpk (works with Gurobi 6.04)
bio_sc_media = testYeastModel(bio,2,0,0,0,1);
% bio model doesn't predict biomass flux with iFF def, so don't do it.
% bio_sc_media_iFF_biomass = testYeastModel(bio,2,1,0,0,1);
...
Y7_kuepfer_media_glu = testYeastModel_kuepfer(Y7,0,0,1);
Y7_kuepfer_media_glu_iFF_biomass = testYeastModel_kuepfer(Y7,1,0,1);

% bio model doesn't predict biomass flux on Kuepfer medium with either
% biomass def, so don't do it. 
% bio_kuepfer_media_glu = testYeastModel_kuepfer(bio,0,0,1); 
% bio_kuepfer_media_glu_iFF_biomass = testYeastModel_kuepfer(bio,1,0,1);
...
Y7_kuepfer_media_gal = testYeastModel_kuepfer(Y7,0,1,1);
Y7_kuepfer_media_gal_iFF_biomass = testYeastModel_kuepfer(Y7,1,1,1);

% bio model doesn't predict biomass flux on Kuepfer medium with either
% biomass def, so don't do it. 
% bio_kuepfer_media_gal = testYeastModel_kuepfer(bio,0,1,1); 
% bio_kuepfer_media_gal_iFF_biomass = testYeastModel_kuepfer(bio,1,1,1);
...
Y7_kuepfer_media_gly = testYeastModel_kuepfer(Y7,0,2,1);
Y7_kuepfer_media_gly_iFF_biomass = testYeastModel_kuepfer(Y7,1,2,1);

% bio model doesn't have glycerol exchange rxn, so don't do it. 
% bio_kuepfer_media_gly = testYeastModel_kuepfer(bio,0,2,1); 
% bio_kuepfer_media_gly_iFF_biomass = testYeastModel_kuepfer(bio,1,2,1);
...
Y7_kuepfer_media_eth = testYeastModel_kuepfer(Y7,0,3,1);
Y7_kuepfer_media_eth_iFF_biomass = testYeastModel_kuepfer(Y7,1,3,1);

% bio model doesn't predict biomass flux on Kuepfer medium with either
% biomass def, so don't do it. 
% bio_kuepfer_media_eth = testYeastModel_kuepfer(bio,0,3,1); 
% bio_kuepfer_media_eth_iFF_biomass = testYeastModel_kuepfer(bio,1,3,1);
...
% iFF_double_KOs = ../doubles/testDoubles( iFF );
% iND_double_KOs = ../doubles/testDoubles( iND );
% iIN_double_KOs = ../doubles/testDoubles( iIN );
% iMM_double_KOs = ../doubles/testDoubles( iMM );
% Y4_double_KOs = ../doubles/testDoubles( Y4 );
% iAZ_double_KOs = ../doubles/testDoubles( iAZ );
% iMMbs_double_KOs = ../doubles/testDoubles( bs );
% Y5_double_KOs = ../doubles/testDoubles( Y5 );
% iTO_double_KOs = ../doubles/testDoubles( iTO );
% Y6_double_KOs = ../doubles/testDoubles( Y6 );
%
% save('../doubles/results/double_KOs','-regexp','.+_double_KOs')

load('../doubles/results/double_KOs.mat'); % this is not fast.
...
save('../doubles/results/double_analysis','-regexp','.+_analysis')
